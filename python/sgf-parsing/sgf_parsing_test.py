...
class SgfParsingTest(unittest.TestCase):
    ...
    def test_linear_go_tree(self):
        input_string = "(;FF[4]C[root]SZ[19];B[aa];W[ab])"
        expected = SgfTree(
            properties={"FF": ["4"], "C": ["root"], "SZ": ["19"]},
            children=[SgfTree(properties={"B": ["aa"]}, children=[SgfTree({"W": ["ab"]})])],
        )
        self.assertEqual(parse(input_string), expected)

    def test_nested_go_tree(self):
        input_string = "(;FF[4](;B[aa];W[ab])(;B[dd];W[ee]))"
        expected = SgfTree(
            properties={"FF": ["4"]},
            children=[
                SgfTree(properties={"B": ["aa"]}, children=[SgfTree({"W": ["ab"]})]),
                SgfTree(properties={"B": ["dd"]}, children=[SgfTree({"W": ["ee"]})])],
        )
        self.assertEqual(parse(input_string), expected)
